...
const Article = ({findArt}:any) => {
  return (
	<>
	{}
    <article className="max-w-2xl px-6 py-24 mx-auto space-y-12 bg-gray-800 text-gray-50">
	<div className="w-full mx-auto space-y-4 text-center">
		<h1 className="text-4xl font-bold leading-tight md:text-5xl">{findArt?.title}</h1>
		<p className="text-sm text-gray-400">by
			<Link rel="noopener noreferrer" to="/dashboard"  className="underline text-violet-400">
				<span className="mx-1" > {findArt?.author}</span>
			</Link>on
			<time> {findArt?.date} </time>
		</p>
	</div>
	...
	<div className="pt-12 border-t border-gray-700">
		...
		<div className="flex flex-col space-y-4 md:space-y-0 md:space-x-6 md:flex-row">
			<div className="flex flex-col">
				<h4 className="text-2xl text-center font-semibold">{findArt.author}</h4>
				<div className="postContent" dangerouslySetInnerHTML={{__html: findArt?.body}}>
          		</div>
			</div>
		</div>

		<div className="flex justify-center">
			<Link to="/dashboard" className="mt-4 px-4 py-2 text-center font-semibold rounded bg-gray-100 text-gray-800">
					Go to Dashboard
			</Link>
		</div>
